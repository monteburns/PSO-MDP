...
def solve_h2_dispatch_daily(prob, param_dict, var_dict, daily_inputs):
    # Assign inputs
    param_dict["demand"].value = daily_inputs["demand"]
    param_dict["gen"].value = daily_inputs["gen"]
    param_dict["price"].value = daily_inputs["price"]
...
""" THIS IS WHERE THINGS GET UGLYY """
""" WHY IT HAS TO BE DONE, IT WAS PERFECT """
...
def _dispatch_worker_battery(day_idx, num_battery, battery_capacity, battery_c_rate,
                             battery_dc_rate, battery_efficiency, soc_init):
    """Worker solving a single battery dispatch horizon."""
    ...
    result = optimize_dispatch_with_battery(
        daily_demand, daily_gen, soc_init,
        battery_capacity, num_battery, battery_c_rate,
        battery_dc_rate, battery_efficiency, daily_price
    )

    if result is None:
        return (start, end, np.zeros(24), np.full(24, soc_init),
                np.zeros(24), np.zeros(24), soc_init)

    return (start, end, result["electricity_sold_daily"],
            result["battery_soc"][1:], result["battery_charge"],
            result["battery_discharge"], result["battery_soc"][-1])



def _dispatch_worker_h2(day_idx, num_battery, battery_capacity, battery_c_rate,
                        battery_dc_rate, battery_efficiency, hydrogen_capacity,
                        electrolyzer_capacity, electrolyzer_efficiency,
                        hydrogen_price, num_electrolyzer,
                        electricity_to_hydrogen, soc_init, h2_init):
    """Worker solving a single dispatch horizon with hydrogen."""
    global _shared_hourly_production, _shared_hourly_demand, _shared_price
    ...
    result = optimize_dispatch_with_hydrogen(
        daily_demand, daily_gen, soc_init, h2_init,
        battery_capacity, num_battery, battery_c_rate, battery_dc_rate,
        battery_efficiency, hydrogen_capacity, num_electrolyzer,
        electricity_to_hydrogen, electrolyzer_capacity,
        electrolyzer_efficiency, daily_price, hydrogen_price
    )

    if result is None:
        return (start, end, np.zeros(24), np.full(24, soc_init),
                np.zeros(24), np.zeros(24), np.zeros(24), h2_init, soc_init)

    return (start, end, result["electricity_sold_daily"],
            result["battery_soc"][1:], result["hydrogen_production"],
            result["battery_charge"], result["battery_discharge"],
            result["hydrogen_storage"][-1], result["battery_soc"][-1])

def battery_8760_dispatch_parallel(
        hourly_production,
        hourly_demand,
        electricity_price,
        num_battery,
    ...
    args_list = [
        (day, num_battery, battery_capacity, battery_c_rate,
         battery_dc_rate, battery_eff, soc)
        for day in range(365)
    ]
    
    with mp.get_context("fork").Pool(get_available_cpus(64)) as pool:
        results = pool.starmap(_dispatch_worker_battery, args_list)

    for start, end, sold, soc_day, charge_day, discharge_day, _ in results:
        electricity_sold[start:end] = sold
        battery_soc_history[start:end] = soc_day
        battery_charge[start:end] = charge_day
        battery_discharge[start:end] = discharge_day
 
    return electricity_sold, battery_soc_history, battery_charge, battery_discharge
 
 
def hydrogen_8760_dispatch_parallel(
        hourly_production,
        hourly_demand,
        electricity_price,
        num_battery,
    ...
    args_list = [
        (day, num_battery, battery_capacity, battery_c_rate, battery_dc_rate,
         battery_eff, hydrogen_capacity, electrolyzer_capacity,
         electrolyzer_eff, hydrogen_price, num_electrolyzer,
         electricity_to_hydrogen, soc, h2_init)
        for day in range(365)
    ]
 
    
    with mp.get_context("fork").Pool(get_available_cpus(64)) as pool:
        results = pool.starmap(_dispatch_worker_h2, args_list)

    for start, end, sold, soc_day, h2_prod, charge_day, discharge_day, _h2, _ in results:
        electricity_sold[start:end] = sold
...
""" UGLYNESS OH MY GOD STOP IT PLEASEE """
""" I STOPED THATT MY CHILD, POOR..
...
def hydrogen_8760_dispatch(
    ...
    for day in range(365):
        ...
        result = solve_h2_dispatch_daily(prob, param_dict, var_dict, daily_inputs)
...
def objective_function_hydrogen_opt(x, return_details=False):
    ...
    result = hydrogen_8760_dispatch_parallel(
        hourly_production, 
        hourly_demand, 
        electricity_price,
        num_battery,
...
def objective_function_battery_opt(x, return_details=False):
    ...
    result = battery_8760_dispatch_parallel(
        hourly_production,
        hourly_demand,
        electricity_price,
        num_battery,
